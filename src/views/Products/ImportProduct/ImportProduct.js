...
import {fetchProductsRequest} from "../../../epics-reducers/services/fetch-product.duck";
...
class ImportProduct extends Component {
  ...
  componentDidMount() {
    this.state.product.date_create = new Date();
  ...
  handleElement(event) {
    this.state.product[event.target.id] = event.target.value
    this.setState(this.state)
  }

  handleSave() {
    this.state.product.code = new Date().getTime();
    if (!this.state.product.code ||
      !this.state.product.name ||
      !this.state.product.import_price ||
      !this.state.product.price ||
      !this.state.product.archive) {
      return
    }
    this.props.handleSaveData(this.state.product, CONSTANTS.EXIT)
  }
  ...
  handleSaveAndContinue() {
    ...
    this.props.handleSaveData(this.state.product, CONSTANTS.CONTINUE)
  }
    ...
    return (
      ...
      >
        <div className="modal-header">
          ...
          <Button color="success" onClick={this.props.handleShowModal}><i
        ...
        <div className="modal-footer">
          ...
          <Button color="success" className="pull-right" onClick={this.props.handleShowModal}>
